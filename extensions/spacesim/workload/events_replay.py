...
def install_events_replay(
    ...
    *,
    ...
    Returns:
        Number of schedule rows written.
    """
    ...
    for gs_idx in sorted(files):
        ...
        if stage_mode == "symlink":
            os.symlink(src_file.resolve(), staged_path)
        elif stage_mode == "copy":
            shutil.copyfile(src_file, staged_path)
        else:
            raise ValueError(
                f"stage_mode must be 'copy' or 'symlink', got: {stage_mode!r}"
            )
